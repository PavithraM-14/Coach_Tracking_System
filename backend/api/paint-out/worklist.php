...
$stmt = $pdo->prepare(
    "SELECT c.id AS coach_id, c.coach_number, c.serial_no,
            ...
            pit.paint_in_datetime, fs.paint_in_to_paint_out_days
     ...
     JOIN paint_in_transactions pit ON pit.coach_id = c.id
     LEFT JOIN fixed_schedules fs ON fs.coach_type_id = c.coach_type_id
     WHERE ca.module = 'PAINT_OUT' AND ca.status = 'ASSIGNED' AND ca.assigned_user_id = :user_id
...
$data = array_map(function ($row) {
    return [
        ...
        'predicted_date' => Schedule::addDays(
            $row['paint_in_datetime'],
            $row['paint_in_to_paint_out_days'] !== null ? (int) $row['paint_in_to_paint_out_days'] : null
        ),
    ];
}, $rows);
