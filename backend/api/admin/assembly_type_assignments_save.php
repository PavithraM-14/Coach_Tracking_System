...
// Validate every row references a real, active ASSEMBLY_PRODUCTION user and
// a real coach type before touching the table — a bad id anywhere aborts
// the whole save rather than silently dropping rows.
$validUserIds = array_column(
    $pdo->query("SELECT u.id FROM users u JOIN roles r ON r.id = u.role_id WHERE r.code = 'ASSEMBLY_PRODUCTION' AND u.is_active = 1")->fetchAll(),
    'id'
);
...
foreach ($assignments as $row) {
    ...
    if (!in_array($userId, $validUserIds, true)) {
        Response::error("Invalid or inactive Assembly Production user_id: {$userId}.", 400);
    }
...
try {
    ...
    $pdo->exec('DELETE FROM assembly_type_assignments');

    $insert = $pdo->prepare(
        'INSERT INTO assembly_type_assignments (user_id, coach_type_id, can_in, can_out)
