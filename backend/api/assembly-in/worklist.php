...
$stmt = $pdo->prepare(
    "SELECT c.id AS coach_id, c.coach_number, c.serial_no,
            ...
            pot.paint_out_datetime, fs.paint_out_to_assembly_in_days
     ...
     JOIN paint_out_transactions pot ON pot.coach_id = c.id
...
$data = array_map(function ($row) {
    return [
        ...
        'predicted_date' => Schedule::addDays(
            ...
            $row['paint_out_to_assembly_in_days'] !== null ? (int) $row['paint_out_to_assembly_in_days'] : null
        ),
    ];
}, $rows);
