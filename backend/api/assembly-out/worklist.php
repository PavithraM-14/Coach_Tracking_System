...
$stmt = $pdo->prepare(
    "SELECT c.id AS coach_id, c.coach_number, c.serial_no,
            ...
            ait.assembly_in_datetime, fs.assembly_in_to_assembly_out_days
     ...
     JOIN assembly_in_transactions ait ON ait.coach_id = c.id
...
$data = array_map(function ($row) {
    return [
        ...
        'predicted_date' => Schedule::addDays(
            $row['assembly_in_datetime'],
            $row['assembly_in_to_assembly_out_days'] !== null ? (int) $row['assembly_in_to_assembly_out_days'] : null
        ),
    ];
}, $rows);
