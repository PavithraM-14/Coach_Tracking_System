...
// Backs the "Total Assembly Out Records" dashboard tile — every coach that
// has ever reached Assembly Out (whether still waiting or completed).
export function AssemblyOutStagesPage() {
  const [rows, setRows] = useState<StageRow[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [query, setQuery] = useState("");

  useEffect(() => {
    Promise.all([getAssemblyOutWorklist(), getAssemblyRecords()])
      .then(([worklistRes, recordsRes]) => {
        ...
        const completed: StageRow[] = recordsRes.data
          .filter((r) => r.assembly_out_datetime !== null)
          .map((r) => ({
            coach_id: r.coach_id,
            coach_number: r.coach_number,
            coach_type: r.coach_type,
            stage: "COMPLETED",
            since: r.assembly_out_datetime,
          }));
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-slate-800">Assembly Out — Coach Stages</h2>
      <p className="mt-1 text-sm text-slate-500">
        Every coach that has reached Assembly Out, and which stage it's at right now.
      </p>
