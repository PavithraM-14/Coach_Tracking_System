...
import { createLockSeal, getLockSealWorklist } from "../api/lockSeal";
import { getMyAssignmentSummary } from "../api/assignments";
import type { AssignmentSummary, LockSealWorklistCoach } from "../types";
...
export function LockSealPage() {
  const [coaches, setCoaches] = useState<LockSealWorklistCoach[] | null>(null);
  ...
  function reload() {
    ...
    getMyAssignmentSummary("LOCK_SEAL").then(setSummary);
  }
  ...
  function handleCoachChange(value: string) {
    ...
    setDate(coach ? toLocalDate(coach.local_outturn_datetime) : undefined);
  }
  ...
  async function handleSubmit() {
    ...
    if (!date) {
      setFieldError("Lock & Seal date is required.");
      return;
    }

    setSubmitting(true);
    try {
      const result = await createLockSeal({
        coach_id: selectedCoachId,
        lock_seal_date: formatDateForDisplay(date),
        lock_seal_time: "00:00",
        remarks: remarks || undefined,
      });
      setSuccess(
        `Lock & Seal recorded for coach ${result.coach_number} on ${result.lock_seal_datetime.slice(0, 10)} — now eligible for Railway Board Outturn.`,
      );
      setSelectedCoachId("");
      setDate(undefined);
      setRemarks("");
      ...
      setApiError(err instanceof ApiError ? err.message : "Failed to record Lock & Seal.");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="max-w-xl">
      <h2 className="text-lg font-semibold text-slate-800">Lock & Seal</h2>
      <p className="mt-1 text-sm text-slate-500">
        Every coach is auto-assigned to you (no cap — one queue, one employee) as soon as Local
        Outturn is recorded. Pick one below — the date starts out matching its Local Outturn date,
        but you can change it before submitting. Submitting opens the coach up for Railway Board
        Outturn.
      </p>
      ...
      <PendingCoachesTable coaches={coaches} emptyMessage="No coaches currently assigned to you for Lock & Seal." />
      ...
      <div className="mt-4">
        ...
        {coaches && coaches.length === 0 && (
          <p className="mt-1 text-sm text-slate-500">No coaches currently assigned to you for Lock & Seal.</p>
        )}
      </div>
      ...
      <div className="mt-5 max-w-xs">
        <DatePickerField label="Lock & Seal Date" value={date} onChange={setDate} />
      </div>
      ...
      <div className="mt-5">
        ...
        >
          {submitting ? "Submitting..." : "Submit Lock & Seal"}
        </button>
      </div>
    </div>
  );
