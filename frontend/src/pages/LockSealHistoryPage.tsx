...
import { getLockSealList } from "../api/lockSeal";
import type { LockSealListRow } from "../types";
import { ApiError } from "../api/client";
import { formatDateOnly } from "../utils/dateFormat";

export function LockSealHistoryPage() {
  const [rows, setRows] = useState<LockSealListRow[] | null>(null);
  ...
  useEffect(() => {
    getLockSealList()
      ...
      .catch((err) => setLoadError(err instanceof ApiError ? err.message : "Failed to load Lock & Seal records."));
  }, []);

  const todayStr = new Date().toLocaleDateString("en-CA");
  const dateFiltered = useMemo(() => {
    if (!rows) return null;
    return todayOnly ? rows.filter((row) => row.lock_seal_datetime.slice(0, 10) === todayStr) : rows;
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-slate-800">Lock & Seal Records{todayOnly && " — Today"}</h2>
      <p className="mt-1 text-sm text-slate-500">
        {todayOnly
          ? "Lock & Seal recorded today, most recent first."
          : "Every Lock & Seal recorded so far, most recent first."}{" "}
        Have a coach assigned to you?{" "}
        <Link to="/lock-seal" className="font-medium text-blue-600 hover:underline">
          Go to Lock & Seal →
        </Link>
      </p>
      ...
      {filtered && filtered.length === 0 && (
        <p className="mt-4 text-sm text-slate-500">
          {dateFiltered && dateFiltered.length === 0
            ? todayOnly
              ? "No Lock & Seal recorded today yet."
              : "No coaches have completed Lock & Seal yet."
        ...
        <div className="mt-2 overflow-x-auto rounded-xl border border-slate-200 bg-white">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((row) => {
                const isToday = !todayOnly && row.lock_seal_datetime.slice(0, 10) === todayStr;
                return (
