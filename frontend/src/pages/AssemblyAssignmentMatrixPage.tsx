...
import { getAssemblyTypeAssignments, saveAssemblyTypeAssignments } from "../api/assemblyAssignments";
...
export function AssemblyAssignmentMatrixPage() {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      const result = await saveAssemblyTypeAssignments(flatCells);
    ...
    <div>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500">Assembly Shop</p>
        </div>
        <button
          type="button"
          onClick={handleSave}
      ...
      {users && users.length === 0 && (
        <p className="mt-6 text-sm text-slate-500">No active Assembly Production employees yet — add one in User Management first.</p>
      )}
