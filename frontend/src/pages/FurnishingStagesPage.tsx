...
import { getFurnishingInWorklist, getFurnishingInList } from "../api/furnishingIn";
import { ApiError } from "../api/client";
import { formatDateOnly } from "../utils/dateFormat";

type Stage = "PENDING" | "AT_FURNISHING_IN" | "MOVED_TO_PAINT_IN";
...
const STAGE_ORDER: Record<Stage, number> = { PENDING: 0, AT_FURNISHING_IN: 1, MOVED_TO_PAINT_IN: 2 };

const STAGE_LABEL: Record<Stage, string> = {
  PENDING: "Pending Furnishing In",
  AT_FURNISHING_IN: "At Furnishing In",
  MOVED_TO_PAINT_IN: "Moved to Paint In",
};

const STAGE_COLOR: Record<Stage, string> = {
  PENDING: "bg-amber-100 text-amber-700",
  AT_FURNISHING_IN: "bg-blue-100 text-blue-700",
  MOVED_TO_PAINT_IN: "bg-green-100 text-green-700",
};

// Backs the "Total Furnishing In Records" dashboard tile — every coach
// that has ever reached Furnishing In (whether still waiting, sitting at
// Furnishing In, or already moved on to Paint In), one row each, so an
// employee can see exactly where every one of those coaches stands now.
export function FurnishingStagesPage() {
  const [rows, setRows] = useState<StageRow[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [query, setQuery] = useState("");

  useEffect(() => {
    Promise.all([getFurnishingInWorklist(), getFurnishingInList()])
      .then(([worklistRes, listRes]) => {
        const pending: StageRow[] = worklistRes.data.map((c) => ({
          ...
          stage: "PENDING",
          since: c.shell_outturn_datetime,
        }));
        const completed: StageRow[] = listRes.data.map((r) => ({
          coach_id: r.coach_id,
          coach_number: r.coach_number,
          coach_type: r.coach_type,
          stage: r.status === "PAINT_IN" ? "MOVED_TO_PAINT_IN" : "AT_FURNISHING_IN",
          since: r.status === "PAINT_IN" && r.paint_in_datetime ? r.paint_in_datetime : r.furnishing_in_datetime,
        }));
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-slate-800">Furnishing In — Coach Stages</h2>
      <p className="mt-1 text-sm text-slate-500">
        Every coach that has reached Furnishing In, and which stage it's at right now.
      </p>
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((row) => (
                <tr key={row.coach_id}>
                  ...
                  <td className="px-4 py-2">{formatDateOnly(row.since)}</td>
                </tr>
              ))}
            </tbody>
          </table>
