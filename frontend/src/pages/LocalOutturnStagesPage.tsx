...
import { getLocalOutturnWorklist, getLocalOutturnList } from "../api/localOutturn";
...
// Backs the "Total Local Outturn Records" dashboard tile — every coach that
// has ever reached Local Outturn (whether still waiting or completed).
export function LocalOutturnStagesPage() {
  const [rows, setRows] = useState<StageRow[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [query, setQuery] = useState("");

  useEffect(() => {
    Promise.all([getLocalOutturnWorklist(), getLocalOutturnList()])
      .then(([worklistRes, listRes]) => {
        ...
        const completed: StageRow[] = listRes.data.map((r) => ({
          coach_id: r.local_outturn_id,
          coach_number: r.coach_number,
          coach_type: r.coach_type,
          stage: "COMPLETED",
          since: r.local_outturn_datetime,
        }));
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-slate-800">Local Outturn — Coach Stages</h2>
      <p className="mt-1 text-sm text-slate-500">
        Every coach that has reached Local Outturn, and which stage it's at right now.
      </p>
