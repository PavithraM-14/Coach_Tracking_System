...
export function FurnishingInHistoryPage() {
  ...
  const dateFiltered = useMemo(() => {
    if (!rows) return null;
    return todayOnly ? rows.filter((row) => row.furnishing_in_datetime.slice(0, 10) === todayStr) : rows;
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-slate-500">
        {todayOnly
          ? "Furnishing In recorded today, most recent first."
          : "Every Furnishing In recorded so far, most recent first."}{" "}
        Have a coach assigned to you?{" "}
        <Link to="/furnishing-in" className="font-medium text-blue-600 hover:underline">
          Go to Furnishing In Entry →
        </Link>
      </p>
      ...
      {filtered && filtered.length === 0 && (
        <p className="mt-4 text-sm text-slate-500">
          {dateFiltered && dateFiltered.length === 0
            ? todayOnly
              ? "No Furnishing In recorded today yet."
              : "No coaches have entered Furnishing yet."
        ...
        <div className="mt-2 overflow-x-auto rounded-xl border border-slate-200 bg-white">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filtered.map((row) => {
                const isToday = !todayOnly && row.furnishing_in_datetime.slice(0, 10) === todayStr;
                return (
                  <tr key={row.furnishing_in_id}>
                    ...
                    <td className="px-4 py-2">
                      {formatDateOnly(row.furnishing_in_datetime)}
                      {isToday && (
