...
import { getPhysicalDispatchWorklist, getPhysicalDispatchList } from "../api/physicalDispatch";
...
// Backs the "Total Dispatch Records" dashboard tile — every coach that has
// ever reached Physical Dispatch (whether still waiting or completed).
export function PhysicalDispatchStagesPage() {
  const [rows, setRows] = useState<StageRow[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [query, setQuery] = useState("");

  useEffect(() => {
    Promise.all([getPhysicalDispatchWorklist(), getPhysicalDispatchList()])
      .then(([worklistRes, listRes]) => {
        ...
        const completed: StageRow[] = listRes.data.map((r) => ({
          coach_id: r.physical_dispatch_id,
          coach_number: r.coach_number,
          coach_type: r.coach_type,
          stage: "COMPLETED",
          since: r.dispatch_datetime,
        }));
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-slate-800">Physical Dispatch — Coach Stages</h2>
      <p className="mt-1 text-sm text-slate-500">
        Every coach that has reached Physical Dispatch, and which stage it's at right now.
      </p>
