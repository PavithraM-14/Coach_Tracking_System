...
import { getLockSealWorklist, getLockSealList } from "../api/lockSeal";
...
// Backs the "Total Lock & Seal Records" dashboard tile — every coach that
// has ever reached Lock & Seal (whether still waiting or completed).
export function LockSealStagesPage() {
  const [rows, setRows] = useState<StageRow[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [query, setQuery] = useState("");

  useEffect(() => {
    Promise.all([getLockSealWorklist(), getLockSealList()])
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-slate-800">Lock & Seal — Coach Stages</h2>
      <p className="mt-1 text-sm text-slate-500">
        Every coach that has reached Lock & Seal, and which stage it's at right now.
      </p>
