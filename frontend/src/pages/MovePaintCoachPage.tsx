...
import { getPaintLines, movePaintCoach } from "../api/paintIn";
import type { PaintLine } from "../types";
import { ApiError } from "../api/client";
import { ValidationMessage } from "../components/ui/ValidationMessage";

// Split out of PaintInPage — moving an already-placed coach to a different
// slot is a separate, occasional maintenance action (freeing up space,
// reorganizing a line), not part of recording a new coach's Paint In.
export function MovePaintCoachPage() {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-slate-800">Move Coach — Paint Lines</h2>
      <p className="mt-1 text-sm text-slate-500">
        Pick up a coach from an occupied slot and drop it into a different line/slot within the
        Paint pool — e.g. to free up space for a higher-priority coach.
      </p>
      ...
      {occupiedSlots.length === 0 && lines && (
        <p className="mt-4 text-sm text-slate-500">No coaches currently occupy a Paint line slot.</p>
      )}
